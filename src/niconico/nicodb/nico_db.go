...
type NicoDB struct {
	db            *sql.DB
	stInsertMedia *sql.Stmt
	mtx           sync.Mutex
}

func Open(ctx context.Context, name string) (nicodb *NicoDB, err error) {
	...
	stInsertMedia, err := db.PrepareContext(ctx,
		`INSERT OR IGNORE INTO media
		(seqno, position, duration, bandwidth, size, data) VALUES 
		(    ?,        ?,        ?,         ?,    ?,    ?)`,
	)
	if err != nil {
		db.Close()
	...
	nicodb = &NicoDB{
		db:            db,
		stInsertMedia: stInsertMedia,
	}

	return
}
func (ndb *NicoDB) Close() {
	ndb.mtx.Lock()
	defer ndb.mtx.Unlock()

	if ndb.stInsertMedia != nil {
		ndb.stInsertMedia.Close()
	}
	if ndb.db != nil {
		ndb.db.Close()
	}
...
func (ndb *NicoDB) FindBySeqNo(ctx context.Context, seqNo uint64) bool {
	ndb.mtx.Lock()
	defer ndb.mtx.Unlock()

	var val uint64
	err := ndb.db.QueryRowContext(ctx, `SELECT seqno FROM media WHERE seqno = ?`, seqNo).Scan(&val)
	if err != nil {
		return false
	}
...
func (ndb *NicoDB) GetNextPosition(ctx context.Context) (position float64) {
	ndb.mtx.Lock()
	defer ndb.mtx.Unlock()

	rows, err := ndb.db.QueryContext(ctx, `SELECT seqno, position FROM media ORDER BY seqno`)
	if err != nil {
		fmt.Println(err)
		return
	}
...
// InsertMedia HLSのチャンクをDBに入れる
func (ndb *NicoDB) InsertMedia(seqNo uint64, position, duration float64, bandwidth, size int64, data []byte) error {
	ndb.mtx.Lock()
	defer ndb.mtx.Unlock()

	_, err := ndb.stInsertMedia.Exec(seqNo, position, duration, bandwidth, size, data)
	return err
}

/*
func (db *NicoDB) dbCreate() (err error) {
	hls.dbMtx.Lock()
